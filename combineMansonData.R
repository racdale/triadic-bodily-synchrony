#mansonDataAll = read.xlsx('Manson_Convo_PD_data.xlsx',sheetIndex=1,startRow=2)
...
mansonData = mansonDataAll #[mansonDataAll$chairs=='LC'|mansonDataAll$chairs=='CR'|mansonDataAll$chairs=='LR',] # in case we want to focus on one chair ordering
...
for (i in 1:dim(wccresNew)[1]) { # integrating Manson data with body correlations (windowed correlation = wcc)
  ...
  mansonData[mansonData$chairs==wccres[i,]$typ & mansonData$triad==triad,21:25] = wccresNew[i,4:8]
  mansonData[mansonData$chairs==reverseString(wccres[i,]$typ) & mansonData$triad==triad,21:25] = wccresNew[i,4:8]
}

# let's retrieve the windowed correlation scores and treat this as a repeatedmeasures setup... multiple observations in 10s segments
...
laughDat = c()
# a slow and lame loop, I know... but we just do it once and we're done
for (i in 1:l) {
  print(i/l)
